    ...
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Form->postLink(
                ...
                ['confirm' => __('Are you sure you want to delete # {0}?', $estimate->id), 'class' => 'side-nav-item']
            ) ?>
    ...
    <div class="column column-80">
        <div class="estimates form content">
            ...
            <fieldset>
                ...
                ?>
                <div style="display: flex; align-items: flex-end; gap: 8px; margin-bottom: 1.5rem;">
                    <div style="flex-grow: 1;">
                        <?= $this->Form->control('customer_id', ['options' => $customers, 'id' => 'customer-id', 'class' => 'form-control']) ?>
                    </div>
                    <button type="button" class="global-quick-add-btn button button-outline" data-url="/customers/index?popup=1" data-target-dropdown="customer-id" style="padding: 0.65rem 0.8rem;" title="Search/Pick Customer">
                        <i class="fa fa-search"></i>
                    </button>
                    <button type="button" class="global-quick-add-btn button button-outline" data-url="/customers/add?popup=1" data-target-dropdown="customer-id" style="padding: 0.65rem 0.8rem;" title="Add New Customer">
...
<script>
    ...
    const existingItems = <?= json_encode($estimate->estimate_items ?: []) ?>;
    let rowIndex = 0;
    ...
    function addRow(data = null) {
        ...
        let productHtml = `<input type="hidden" name="estimate_items[${rowIndex}][id]" value="${itemId}">`;
        productHtml += `<select name="estimate_items[${rowIndex}][product_id]" id="product-select-${rowIndex}" onchange="updateProduct(${rowIndex}, this.value)" class="form-control"><option value="">Custom/Other</option>`;
        ...
        tr.innerHTML = `
            ...
            <td>${accountHtml}</td>
            <td><input type="number" name="estimate_items[${rowIndex}][quantity]" id="qty-${rowIndex}" step="1" value="${qty}" onchange="calculateLine(${rowIndex})" class="form-control"></td>
            <td><input type="number" name="estimate_items[${rowIndex}][unit_price]" id="price-${rowIndex}" step="0.01" value="${price}" onchange="calculateLine(${rowIndex})" class="form-control"></td>
            <td><input type="text" name="estimate_items[${rowIndex}][hs_code]" id="hs-code-${rowIndex}" class="form-control" value="${hsCode}" placeholder="HS Code" onchange="handleHsCode(${rowIndex})"></td>
            <td><input type="text" name="estimate_items[${rowIndex}][vat_type]" id="vat-type-${rowIndex}" class="form-control" value="${vatType}" placeholder="Type"></td>
            <td><input type="number" name="estimate_items[${rowIndex}][vat_rate]" id="vat-rate-${rowIndex}" step="0.01" value="${vatRate}" onchange="calculateLine(${rowIndex})" class="form-control"></td>
            <td><input type="number" name="estimate_items[${rowIndex}][vat_amount]" id="vat-amount-${rowIndex}" step="0.01" value="${vatAmount}" readonly class="form-control" style="background: #f8fafc; border: none;"></td>
            <td><input type="number" name="estimate_items[${rowIndex}][line_total]" id="total-${rowIndex}" step="0.01" value="${lineTotal}" readonly class="form-control" style="background: #f8fafc; border: none; font-weight: 600;"></td>
