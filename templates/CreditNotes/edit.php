    ...
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Form->postLink(
                ...
                ['confirm' => __('Are you sure you want to delete # {0}?', $creditNote->id), 'class' => 'side-nav-item']
            ) ?>
    ...
    <div class="column column-80">
        <div class="creditNotes form content">
            ...
            <fieldset>
                ...
                <h4 style="margin-top: 2rem;">Credit Note Items</h4>
...
<script>
    ...
    const existingItems = <?= json_encode($creditNote->credit_note_items ?: []) ?>;
    let rowIndex = 0;
    ...
    function addRow(data = null) {
        ...
        let productHtml = `<input type="hidden" name="credit_note_items[${rowIndex}][id]" value="${itemId}">`;
        productHtml += `<select name="credit_note_items[${rowIndex}][product_id]" onchange="updateProduct(${rowIndex}, this.value)" class="form-control"><option value="">Custom/Other</option>`;
        ...
        let accountHtml = `<select name="credit_note_items[${rowIndex}][account_id]" id="account-${rowIndex}" class="form-control">`;
        ...
        tr.innerHTML = `
            <td>${productHtml}</td>
            <td>${accountHtml}</td>
            <td><input type="number" name="credit_note_items[${rowIndex}][quantity]" id="qty-${rowIndex}" step="1" value="${qty}" onchange="calculateLine(${rowIndex})" class="form-control"></td>
            <td><input type="number" name="credit_note_items[${rowIndex}][unit_price]" id="price-${rowIndex}" step="0.01" value="${price}" onchange="calculateLine(${rowIndex})" class="form-control"></td>
            <td><input type="number" name="credit_note_items[${rowIndex}][vat_rate]" id="vat-rate-${rowIndex}" step="0.01" value="${vatRate}" onchange="calculateLine(${rowIndex})" class="form-control"></td>
            <td><input type="number" name="credit_note_items[${rowIndex}][vat_amount]" id="vat-amount-${rowIndex}" step="0.01" value="${vatAmount}" readonly class="form-control" style="background: #f8fafc; border: none;"></td>
            <td><input type="number" name="credit_note_items[${rowIndex}][line_total]" id="total-${rowIndex}" step="0.01" value="${lineTotal}" readonly class="form-control" style="background: #f8fafc; border: none; font-weight: 600;"></td>
